...
export default function AssistantChatButton({ open, onClick }: AssistantChatButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={`fixed bottom-6 right-6 flex h-16 w-16 items-center justify-center rounded-full border border-[#D4AF37]/40 bg-[#0A0F1A] text-white shadow-2xl transition hover:-translate-y-0.5 hover:border-[#D4AF37] hover:bg-[#003A8C] focus:outline-none focus:ring-4 focus:ring-[#D4AF37]/25 ${
